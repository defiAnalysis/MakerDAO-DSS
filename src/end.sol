...
*/
//此模块协调关机。简而言之，关机关闭了系统并补偿了Dai持有人。
//此过程可以在升级(Dai迭代)期间发生，也可以在代码和设计中出现实现缺陷时出于安全原因发生。

//End的目的是协调系统的关闭。这是一个复杂且有状态的过程，发生在下面九个主要步骤中
//1.cage()
//  1.1. 这个过程开始于冻结系统，并锁定每种抵押品的价格。
//       1.1.1 停止存放抵押品和从金库里提取Dai的能力

//接下来，系统将停止所有当前的Flap/Flop拍卖，从而允许取消对单个拍卖的调用，并调用相应合约的yank()函数。
// 这些拍卖被冻结和取消的原因之一是因为关闭程序的目的是将系统盈余或系统债务转嫁给Dai。 
//此外，无法保证关闭期间MKR的价值，因此无法依赖MKR市场价值的机制，这意味着没有理由继续进行会影响MKR供应的拍卖。 
contract End is LibNote {
    ...
    //经过处理后的Dai正常/稳定币供应正常，系统盈余/赤字已被吸收。
    ...
    //cage -锁定系统并启动关机。这是通过冻结面向用户的操作、取消flap和flop拍卖、
    //锁定剩余的系统合同、禁用可能干扰解决过程的某些治理操作以及开始冷却期来实现的。
    ...
    //从已结算的保管库中删除（剩余）抵押品。 仅在保险柜中没有债务后才会发生。
    ...
    //将Dai锁定在Cash之前/将一些稳定币放入bag以准备现金。
